...
import cssBuddhiyoga from './buddhiyogaBlockInformaion.css';
import css99kaPhere from './99kapherBlockInformaion.css';


const CellInformation = ({setrotation,rotation,excerpt,readExcerpt,readpostId,cellName,postButton,postName,postId,navigation,postIndex,postPlayIndex}) => {
    ...
    // console.log("readpostList.length"+readpostList.length);
  ...
  useEffect(()=>{
    ...
    {
        // console.log("cssBuddhiyoga");
      setCssFile(cssBuddhiyoga)
    }
    else{
        // console.log("css99kaPhere");
...
return(
    ...
    <View style={{flexDirection: 'row', justifyContent: 'flex-start', marginLeft: 15, width: '50%', }}>
        ...
        <TouchableWithoutFeedback onPress={()=>{setreadStatus(false)}}> 
            ...
            <View style={{ backgroundColor: readStatus?'#d7c6c1':'#594039',paddingHorizontal: 8, paddingVertical: 6, width: 56, flex: 1, marginHorizontal: 2, borderTopLeftRadius: 5, borderTopRightRadius: 5 }}>
                <Text style={{textAlign: 'center', fontWeight: '500', color: readStatus?'#594039':'#fff'}}>Play</Text>
            </View>

        </TouchableWithoutFeedback>
        <TouchableWithoutFeedback onPress={()=>{setreadStatus(true)}}> 

            <View style={{ backgroundColor:readStatus?'#594039':'#d7c6c1',paddingHorizontal: 8, paddingVertical: 6, width: 56, flex: 1, marginHorizontal: 2, borderTopLeftRadius: 5, borderTopRightRadius: 5 }}>
                <Text  style={{textAlign: 'center', fontWeight: '500', color: readStatus?'#fff':'#594039'}}>Read</Text>
            </View>
            ...
            <>
                 {
                    readpostList.length < 0 ?
                            <View>
                            <Text style={{alignContent:'flex-start',paddingHorizontal:"5%",paddingTop:"1%",color: 'black',fontSize: 13,}} ellipsizeMode='tail'>Double tap any cell to know more1</Text>
                        </View>
                        :  
                        <>
