...
const Game=(props) => {  
  ...
  useEffect(()=>{
    // alert(global.config.GAME_DATA.name)
...
{
  getData('@saveSateby').then(async(data)=>{
...
const [postButton,setPostButton]=useState(false);
  ...
  const saveStatesFunc =async(postName)=>{
      ...
      storeData('@saveSateby',saveStates);
      playerMoveby.push(saveStates);
      var storageData=await getData('@playerMoveby');
      if(storageData===null)
      {
        storeData('@playerMoveby',playerMoveby);
      }
      else{
        storageData=JSON.parse(storageData);
        storageData.push(saveStates);
        storeData('@playerMoveby',storageData);
      }
      bufferStates.deviceInformaion=device;
      bufferPlayerMoves.push(bufferStates);
      var bufferStorageData=await getData('@bufferPlayerMoveby');
      if(bufferStorageData===null)
      {
        storeData('@bufferPlayerMoveby',bufferPlayerMoves);
      }
      else{
        bufferStorageData=JSON.parse(bufferStorageData);
        bufferStorageData.push(bufferStates);
         storeData('@bufferPlayerMoveby',bufferStorageData);
      }
  ...
  if(postReadIdCurrent.current !==undefined){
    ...
    const magicButton=()=>{
      props.navigation.navigate('recentMoves');
    }
...
return (
    ...
    <SafeAreaView  style={[cssFile.safeAreaView,{width: width,height: height}]}>
      <ScrollView>
    <Hamburger navigation={props.navigation} infoFunction={magicButton} resetStatus={true} magicStatus={magicStatus} howtoplayFunction={howtoplayFunction} />
    <View style={[cssFile.gameView,{width: width}]} >
    <Animated.View style={cssFile.gameContainer}>
    <Animated.View style={[cssFile.imageView,{width:width}]}>
    {/* <ReactNativeZoomableView
       ...
       <TapGestureHandler
          ...
          onActivated={(e) => {
            ...
            <Animated.Image source={{uri:imageUrl}} style={cssFile.imageStyle}></Animated.Image>
        </TapGestureHandler>
      ...
      <Animated.Image ref={diceFaceFrame} source={diceFace} style={{width:"95%",height:"100%"}} resizeMode="contain"/>
      </TouchableOpacity>
      </Animated.View>
      </View>
    </Animated.View>
