...
// import style from './buddhiyoga.module.css';
import ReactNativeZoomableView from '@openspacelabs/react-native-zoomable-view/src/ReactNativeZoomableView';
import { TouchableOpacity } from 'react-native';
import BlockInformation from '../components/Celllinformation_01_11_2023';
...
const stateChangePawn = async()=>
{
  ...
  const diceRoll=async()=>{
    ...
    if(specialMove.current > 0 || specialMove.current < 0 )
    {
      ...
      setDiceFace(diceImage[faceDice].imageurl)
        ...
        currentDiceFace.current=faceDice + 1;
    }
  ...
  const initiatePawnMovement = async ()=>{
   ...
   if (iDisplacement.current == 0) {
     ...
     // console.log(playerPositions[player.current.position].info.movement.specialMove);
  if(iDisplacement.current==-62 || iDisplacement.current==0 || (player.current.position + iDisplacement.current)>110){
   setTimeout(() => {
     setDiceFace(diceImage[6].imageurl)
     dice.current.iDiceFace=diceImage[6].imageurl
   }, 1000);
...
return (
    ...
    <Animated.View style={[cssFile.imageView]}>
